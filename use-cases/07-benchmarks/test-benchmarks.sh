...
#
python 01-tbtxt2bin.py soc-LiveJournal1-noheader.txt soc-LiveJournal1.table
RETVAL=$?
if [ $RETVAL -ne 0 ]; then
    echo "***" `date` "01 ERROR"
    exit $RETVAL
...
#
python 07-join.py soc-LiveJournal1.table soc-LiveJournal1_10k.txt
RETVAL=$?
if [ $RETVAL -ne 0 ]; then
...
# join t2 and t1 on first columns
#
python 07-join.py soc-LiveJournal1.table soc-LiveJournal1_all.txt
RETVAL=$?
if [ $RETVAL -ne 0 ]; then
...
#
python 08-groupaggr.py input/posts_scores.tsv
RETVAL=$?
if [ $RETVAL -ne 0 ]; then
    echo "***" `date` "08 ERROR"
    exit $RETVAL
...
#
python 09-isnextk.py input/posts_scores.tsv
RETVAL=$?
if [ $RETVAL -ne 0 ]; then
    echo "***" `date` "09 ERROR"
    exit $RETVAL
...
# for each post, finds the next post by the same user in chronological order
#
#python 10-graphseq.py input/posts_scores.tsv
RETVAL=$?
if [ $RETVAL -ne 0 ]; then
    echo "***" `date` "10 ERROR"
    exit $RETVAL
...
#
python 11-graphgroup.py input/posts_scores.tsv input/tags.tsv
RETVAL=$?
if [ $RETVAL -ne 0 ]; then
    echo "***" `date` "11 ERROR"
    exit $RETVAL
...
#
#python 12-usecase.py input/posts_tags.tsv out.bin
RETVAL=$?
if [ $RETVAL -ne 0 ]; then
    echo "***" `date` "12 ERROR"
    exit $RETVAL
...
#
#python 14-usecase-complete.py input/posts_tags.tsv out.bin
RETVAL=$?
if [ $RETVAL -ne 0 ]; then
    echo "***" `date` "14 ERROR"
    exit $RETVAL
...
#
python 50-grtxt2bin.py soc-LiveJournal1-noheader.txt soc-LiveJournal1.graph
RETVAL=$?
if [ $RETVAL -ne 0 ]; then
    echo "***" `date` "50 ERROR"
    exit $RETVAL
...
#
python 52-nettxt2bin.py soc-LiveJournal1-noheader.txt soc-LiveJournal1.net
RETVAL=$?
if [ $RETVAL -ne 0 ]; then
    echo "***" `date` "52 ERROR"
    exit $RETVAL
