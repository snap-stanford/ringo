    ...
    def __getattr__(self, name):
        ...
        if hasattr(Obj, name):
            def wrapper(*args, **kwargs):
                def func(Ringo, *args, **kwargs):
                    return getattr(Obj, name)(*args, **kwargs)
                return registerOp(name, False)(func)(self.Ringo, *args, **kwargs)
            return wrapper
...
#            return wrapper
    ...
    def DelZeroDegNodes(GraphId):
        Graph = self.Objects[GraphId]
        snap.DelZeroDegNodes(Graph)
        return RingoObject(GraphId, self)

    @registerOp('DelDegKNodes')
    def DelDegKNodes(GraphId, OutDetK, InDegK):
        Graph = self.Objects[GraphId]
    ...
    def GetTreeRootNId(GraphId):
        Graph = self.Objects[GraphId]
        NId = snap.GetTreeRootNId(Graph)
        return NId

    @registerOp('GetTreeSig')
    def GetTreeSig(GraphId, RootNId):
        Graph = self.Objects[GraphId]
        Sig = snap.TIntV()
    ...
    @registerOp('GetAnf')
    def GetAnf(GraphId, SrcNId, MxDist, IsDir, NApprox = 32):
        Graph = self.Objects[GraphId]
    ...
    @registerOp('GetAnf')
    def GetAnf(GraphId, MxDist, IsDir, NApprox = 32):
        Graph = self.Objects[GraphId]
        DistNbrsV = snap.TIntFltKdV()
    ...
    def GetAnfEffDiam(GraphId, IsDir, Percentile, NApprox):
        Graph = self.Objects[GraphId]
        Len = snap.GetAnfEffDiam(Graph, snap.TBool(IsDir), Percentile, NApprox)
        return Len

    @registerOp('GetAnfEffDiam', False)
    def GetAnfEffDiam(GraphId, NRuns = 1, NApprox = -1):
        Graph = self.Objects[GraphId]
        Len = snap.GetAnfEffDiam(Graph, NRuns, NApprox)
        return Len

    @registerOp('GetBfsTree')
    def GetBfsTree(GraphId, StartNId, FollowOut, FollowIn):
        Graph = self.Objects[GraphId]
    ...
    def GetNodesAtHops(GraphId, StartNId, IsDir):
        Graph = self.Objects[GraphId]
    ...
    def GetShortPath(GraphId, SrcNId, DstNId, IsDir = False):
        Graph = self.Objects[GraphId]
        Len = snap.GetShortPath(Graph, SrcNId, DstNId, snap.TBool(IsDir))
        return Len

    @registerOp('GetShortPath')
    def GetShortPath(GraphId, SrcNId, IsDir = False, MaxDist = snap.TInt.Mx):
        Graph = self.Objects[GraphId]
    ...
    def GetEigenVectorCentr(GraphId, Eps = 1e-4, MaxIter = 100):
        Graph = self.Objects[GraphId]
    ...
    def GetModularity(GraphId, NIdVId, GEdges = -1):
        Graph = self.Objects[GraphId]
        NIdV = self.Objects[NIdVId]
    ...
    def GetEdgesInOut(GraphId, NIdVId):
        Graph = self.Objects[GraphId]
        NIdV = self.Objects[NIdVId]
        snap.GetEdgesInOut(Graph, NIdV, Ret)
        return Ret

    @registerOp('GetBiConSzCnt')
    def GetBiConSzCnt(GraphId):
        Graph = self.Objects[GraphId]
        SzCntV = snap.TIntPrV()
    ...
    def GenRewire(GraphId, NSwitch = 100, Rnd = snap.TInt.Rnd):
        Graph = self.Objects[GraphId]
    ...
    def GenPrefAttach(Nodes, NodeOutDeg, Rnd = snap.TInt.Rnd):
    ...
    def GenGeoPrefAttach(NumNodes, NumEdges, Beta, Rnd = snap.TInt.Rnd):
        Graph = snap.GenGeoPrefAttach(NumNodes, NumEdges, Beta, Rnd)
        GraphId = self.__UpdateObjects(Graph, )
        return RingoObject(GraphId, self)

    @registerOp('GenSmallWorld')
    def GenSmallWorld(Nodes, NodeOutDeg, RewireProb, Rnd = snap.TInt.Rnd):
    ...
    def GenCopyModel(Nodes, Beta, Rnd = snap.TInt.Rnd):
        Graph = snap.GenCopyModel(Nodes, Beta, Rnd)
        GraphId = self.__UpdateObjects(Graph, )
        return RingoObject(GraphId, self)

    @registerOp('GenRMat')
    def GenRMat(Nodes, Edges, A, B, C, Rnd = snap.TInt.Rnd):
    ...
    @registerOp('GenGrid')
    def GenGrid(GraphTypeId, Rows, Cols, IsDir = True):
    ...
    @registerOp('GenStar')
    def GenStar(GraphTypeId, Nodes, IsDir = True):
        GraphType = self.Objects[GraphTypeId]
    ...
    def GenCircle(GraphTypeId, Nodes, OutDegree, IsDir = True):
        GraphType = self.Objects[GraphTypeId]
    ...
    @registerOp('GenTree')
    def GenTree(GraphTypeId, Fanout, Levels, IsDir = True, ChildPointsToParent = True):
        GraphType = self.Objects[GraphTypeId]
    ...
    def GenBaraHierar(GraphTypeId, Levels, IsDir = True):
        GraphType = self.Objects[GraphTypeId]
    ...
    def GenRndGnm(GraphTypeId, Nodes, Edges, IsDir = True, Rnd = snap.TInt.Rnd):
        GraphType = self.Objects[GraphTypeId]
    ...
    def LoadEdgeList(GraphTypeId, InFNm, SrcColId, DstColId, Separator):
        GraphType = self.Objects[GraphTypeId]
    ...
    @registerOp('LoadEdgeList')
    def LoadEdgeList(GraphTypeId, InFNm, SrcColId, DstColId):
        GraphType = self.Objects[GraphTypeId]
    ...
    def LoadEdgeListStr(GraphTypeId, InFNm, SrcColId, DstColId):
        GraphType = self.Objects[GraphTypeId]
    ...
    def LoadConnList(GraphTypeId, InFNm):
        GraphType = self.Objects[GraphTypeId]
    ...
    def LoadConnListStr(GraphTypeId, InFNm):
        GraphType = self.Objects[GraphTypeId]
        StrToNIdH = snap.TStrIntH()
    ...
    def LoadPajek(GraphTypeId, InFNm):
        GraphType = self.Objects[GraphTypeId]
    ...
    @registerOp('SavePajek', False)
    def SavePajek(GraphId, OutFNm, NIdColorHId, NIdLabelHId, EIdColorHId):
        Graph = self.Objects[GraphId]
        NIdColorH = self.Objects[NIdColorHId]
        NIdLabelH = self.Objects[NIdLabelHId]
    ...
    def GetEigVals(GraphId, EigVals):
        Graph = self.Objects[GraphId]
        EigValV = snap.TFltV()
    ...
    def GetInvParticipRate(GraphId, EigVecs, TimeLimit):
        Graph = self.Objects[GraphId]
    ...
    @registerOp('DrawGViz')
    def DrawGViz(GraphId, LayoutId, PItFNm, Desc, NodeLabelHId):
        Graph = self.Objects[GraphId]
    ...
    @registerOp('DrawGViz')
    def DrawGViz(GraphId, LayoutId, PItFNm, Desc = "", NodeLabels = False, NIdColorH = snap.TIntStrH()):
        Graph = self.Objects[GraphId]
        Layout = self.Objects[LayoutId]
        snap.DrawGViz(Graph, Layout, PItFNm, Desc, snap.TBool(NodeLabels), NIdColorH)

    @registerOp('GetKCore')
    def GetKCore(GraphId, K):
        Graph = self.Objects[GraphId]
